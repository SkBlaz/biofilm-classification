...
\title{Biofilm classification analysis pipeline\\User manual}
\version{1.0.0}
...
The execution of the pipeline depends heavily on the information contained in the image names, which should follow the example below.
...
\begin{enumerate}
	\item Clone the source code from the GitHub website into the desired location.
	\item Start the terminal in the above-selected location.
	\item Build the biofilm classification Docker container using the command below.
\end{enumerate}

For Windows users:
\begin{minted}{bat}
c:\my-download-folder\biofilm-classification> build-and-verify.bat
\end{minted}

For Linux users:
\begin{minted}{sh}
# to make the file executable
(/my-download-folder/biofilm-classification) $ chmox +x build-and-verify.sh 
(/my-download-folder/biofilm-classification) $ ./build-and-verify.sh
\end{minted}

Once the Docker image is built and verified (it takes a while), you can run analyses as described below. 
The Docker image will not need to be rebuilt unless changes are made to the source code.
...
\section{Preparation of data for running the classification pipeline}
...
\begin{enumerate}
	...
	\item Create a folder in which you want to save the calculation results. For example, create the subfolder results\_example on the desktop and make a note of the path or address, e.g.\\ /c/Users/my\_name/Desktop/biofilm-classification/results\_example.
	\item (optional) Change the voxel dimensions if necessary: To do this, go to src folder, find the feature generator script and change the following part as desired: \\
	...
	\item Open the source code folder. Search for the file with the name “.env”.
\end{enumerate}
...
One must replace "/c/Users/my\_name/Desktop/biofilm-classification/results\_example" with the corresponding path if you have created the subfolder in a different location than described above. The address must be changed as described above. Again, make sure that you do not change the text after the ":".
...
IMAGINE\_RESULTS=/c/Users/my\_name/Desktop/biofilm-classification/results\_example

\section{Generating features using the classification pipeline}

To generate features with the pipeline, run the following command in the terminal (the parts in<> must be replaced as described above and below and <> as well (see examples below):
...
docker compose 
  ...
  --rm biofilm-classification 1 datafile.tsv 10 generate_features
\end{minted}
...
An example of what kind of results you can expect can be found in the biofilm-classification/examples/results\_example folder that belongs to the pipeline.
...
\section{Classification using the pipeline}
...
\begin{minted}{sh}
docker compose run 
	--rm biofilm-classification datafile.tsv 
...
\begin{minted}{sh}
docker compose run 
    --rm biofilm-classification 1 datafile.tsv 10 reduce_layers 21
\end{minted}
...
\section{List and descriptions of biofilm features calculated by the pipeline\label{apdx:features}}
