...
def _husl_fallback(n):
    ...
    return sns.husl_palette(n, s=.90, l=.55)
...
def plot_umap(df, title, filename, global_palette, global_markers=None, label_col_norm="label_norm"):
    ...
    if X.shape[0] < 2 or X.shape[1] < 1:
        print(f"[WARN] Skipping '{title}': insufficient data for UMAP "
    ...
    plot_df = pd.DataFrame({
        "UMAP1": embedding[:, 0],
        "UMAP2": embedding[:, 1],
        label_col_norm: df[label_col_norm].map(_norm_label).values
    })
    ...
    sns.scatterplot(
        data=plot_df,
        x="UMAP1", y="UMAP2",
        hue=label_col_norm, palette=sub_palette,
...
def process_and_plot_samples(group_name, valid_samples, data, global_palette, global_markers):
    ...
    plot_umap(
        subset,
        f'UMAP visualization ({group_name})',
        f"umap_proj_{group_name}.tif",
        global_palette, global_markers
    )
...
def main():
    ...
    row3 = [s.lower() for s in [
        "Animal clinical", "Animal feed", "Food", "Food",
    ...
    for date in sorted(dates):
        ...
        plot_umap(
            subset,
            f'UMAP visualization ({date})',
            f"umap_proj_{date}.tif",
            global_palette, global_markers
        )
