...
def parse_file_to_df(fname):
    ...
    with open(fname, encoding="cp1251") as inpfn:
        ...
        for line in inpfn:
            ...
            if "Distance (" in line:
                distancesx = [float(x.replace(",", ".")) for x in line.split("\t")[2:]]
                distances.append(distancesx)

            elif "Percent:" in line:
                percx = [float(x.replace(",", ".")) for x in line.split("\t")[2:]]
                percents.append(percx)

    assert len(distances) == len(names) == len(percents)

    for enx in range(len(names)):
        ddl[names[enx]].append(
            {"Distance": distances[enx], "Percent": percents[enx], "Positions": positions[enx], "full_name": full_names[enx]}
        )
    return ddl
...
def average_and_errors(dist_storage):
    ...
    for k, v in dist_storage.items():
        ...
        suma /= len(v)
        ret[k] = {"Distance": [0.5 * i for i in range(0, 21)], "Average": suma, "Max": max_val - suma, "Min": suma - min_val}
    return ret
