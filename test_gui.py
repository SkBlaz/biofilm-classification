...
class TestDirectExecution(unittest.TestCase):
    ...
    def test_training_plan_calls_existing_learning_entry_point(self):
        with temporary_jobs_root():
            ...
            self.assertNotIn("--replication_unit", steps[0].commands[0])

    def test_full_plan_validates_generated_features_without_grouped_cv_options(self):
        with temporary_jobs_root():
            paths = create_job()
            (paths.training_images / labelled_image_name()).touch()
            ...
            self.assertNotIn("--replication-unit", validation_command)
            self.assertNotIn("--nested-cv", validation_command)
...
class TestHTTPAndImage(unittest.TestCase):
    ...
    def test_browser_interface_uses_uploads_jobs_and_zip_downloads(self):
        ...
        self.assertNotIn("/api/replication-options", javascript)
        self.assertNotIn("Unit of replication", html)
